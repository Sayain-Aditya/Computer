...
const SharedQuotation = () => {
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* Minimal Controls - Only show if not clean */}
      {!isClean && (
        <div className="no-print p-2 flex justify-end gap-2">
          <button 
            onClick={() => {
              const cleanUrl = `${window.location.origin}/shared-quotation/${id}?clean=true`
              const message = `Computer Shop Quotation\n\nCustomer: ${quotationData.customer.name}\nTotal: ${formatIndianCurrency(quotationData.totalAmount)}\n\nView PDF: ${cleanUrl}`
